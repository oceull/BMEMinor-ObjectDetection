...
import tensorflow.keras.applications.densenet as tf
...
def train(epoch, model, data_loader, optimizer, criterion, scheduler=None):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        loss.backward()
        optimizer.step()
        losses.append(loss.data.cpu().mean())
    if scheduler:
...
def evaluate(model, data_loader, criterion, need_sigmoid=False):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        loss = criterion(output, target)
        losses.append(loss.cpu().data.mean())
...
def train_graph(epoch, model, data_loader, optimizer, criterion,
    ...
    logging.info("Epoch %d Loss: %.4f" % (epoch,sum(losses) / len(losses)))
