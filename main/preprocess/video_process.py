...
import cv2
import skvideo
skvideo.setFFmpegPath("C:/ffmpeg-4.4.1/bin")
import skvideo.io
...
flags.DEFINE_string("annot_path",
                    curr_path + "/mnt/data/m2cai/m2cai_tool",
                    "Annotation path of surgical videos.")
flags.DEFINE_string("target_path",
                    curr_path + "/mnt/data/m2cai/m2cai_tool/images/test",
...
def extract_frames_from_video(video_path, annot_path, target_path, resize,
    ...
    video_id = video_path.split("/")[-1].split(".")[0].split('\\')[-1]
    ...
    for index, label in zip(indexes, labels):
        logging.info("Saving image dam %s dd " + str(video_id),
