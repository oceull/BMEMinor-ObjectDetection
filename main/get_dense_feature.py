...
flags.DEFINE_string("video_path", curr_path + "/mnt/data/m2cai/m2cai_tool/train_dataset",
                    "Video path.")
...
flags.DEFINE_string("output_path",
                    curr_path + "/mnt/data/m2cai/m2cai_tool/dense_features/train",
...
FLAGS = flags.FLAGS
# PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:2750
...
def get_dense_features(model,
    ...
    model.eval()
    breakpoint()
    dense_features = np.zeros((data_len, 7))
    ...
    for batch_idx, (data, _) in enumerate(data_loader):
        ...
        video_length = outputs.shape[0]
        length = min(1024, video_length)
        # for i in range(batch_idx*batch_size,(batch_idx+1)*batch_size):
        #     for j in range(length -1):
        #         if i%4 >= outputs.shape[0]:
        #             i = outputs.shape[0] - 1
        #             dense_features[i][j] = \
        #                 outputs.data[i][j]
        #             break
...
def main(_):
    ...
    for video_file in video_files:
        ...
        output_file_name = str(os.path.join(FLAGS.output_path, output_file_name))
