...
from dataset import get_dense_graph_loader
from metrics import m2cai_map
from models import get_video_model
from train_utils import evaluate_graph, final_evaluate_graph, train_graph
...
flags.DEFINE_string(
    ...
    curr_path + "/video_features/train_video.pkl",
    "Train data set list.")
flags.DEFINE_string(
    "valid_list",
    # "/mnt/data/m2cai/m2cai_tool/video_features_cropped_long/valid_video.pkl",
    curr_path + "/video_features/valid_video.pkl",
    "Valid data set list.")
flags.DEFINE_string(
    "test_list",
    # "/mnt/data/m2cai/m2cai_tool/video_features_cropped_long/test_video.pkl",
    curr_path + "/video_features/test_video.pkl",
    "Test data set list.")
flags.DEFINE_integer("num_gpu", 1, "Number of gpus to use.")
flags.DEFINE_integer("batch_size", 64, "Batch size.")
flags.DEFINE_float("lr", 0.0000005, "Optimizer Learning Rate.")
flags.DEFINE_float("momentum", 0.5, "Optimizer momentum.")
flags.DEFINE_integer("epoch_num", 1000, "Epoch numbers to train.")
flags.DEFINE_string("save_model_path", curr_path + "/saved/saved_video_graph_12/",
                    "Path to save models.")
flags.DEFINE_string("load_model_path", "", "Save model parameters.")
flags.DEFINE_string("pool_type", "avg", "Temporal pooling type.")
flags.DEFINE_string("gt_path", curr_path + "/mnt/data/m2cai/m2cai_tool/test_dataset/", "Groud truth path.")
flags.DEFINE_boolean("moe", False, "If use MOE in the model.")
flags.DEFINE_string("loss_type", "softmargin", "Either BCE or MultiSoftMargin")
...
def main(_):
    ...
    train_loader, _ = get_dense_graph_loader(
        FLAGS.train_list, batch_size=FLAGS.batch_size, shuffle=True)
    valid_loader, _ = get_dense_graph_loader(
        FLAGS.valid_list, batch_size=FLAGS.batch_size, shuffle=True)
    test_loader, test_data_len = get_dense_graph_loader(
    ...
    for i in range(epoch, FLAGS.epoch_num):
        train_graph(
            ...
            scheduler=scheduler)
        evaluate_graph(
            model, valid_loader, criterion_val, need_sigmoid=need_sigmoid)
        if i % 10 == 0:
            path_to_save = os.path.join(FLAGS.save_model_path,
                                        "params_epoch_%04d.pkl" % i)
            torch.save(model.state_dict(), path_to_save)
    # Final evaluation.
    pred = final_evaluate_graph(model, test_loader, test_data_len,
